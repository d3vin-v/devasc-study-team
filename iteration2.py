...
"""Colorama is used to add color, style, and backgrounds to text outputs. There is an example of this
within the code at line 115f you haven't already changed the code)
Use Fore.x (with x being a color) to add text color
Use Back.x (same as fore) to add colored background
Use Style.x (x = DIM,NORMAL,BRIGHT) I didn't try this option yet 
Use pip install colorama to install the module so you don't get any errors"""
...
while True:
    ...
    vehicle = input(Fore.RED + Back.WHITE + "Enter a vehicle profile from the list above: ")
